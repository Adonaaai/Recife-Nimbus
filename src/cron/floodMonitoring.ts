...
export const monitorJob = async () => {
    ...
    const wasRecentlySent = async(zoneId: number, severity: Severity): Promise<boolean> => {
    ...
    cron.schedule('*/15 * * * *', async () => {
        ...
        try {
           // == Fetch live sensor data from APAC ==========================
            ...
            console.log(`[APAC] ${rainSensors.length} rain sensors | ${riverSensors.length} river stations`);
            ...
            for (const zone of zones) {
                ...
                const zoneRiverSensors: RiverSensor[] = riverSensors.filter((sensor) => {
                    ...
                    return nameMatch
                        ...
                        && sensor.attributes.recent === 's' // must have a recent reading
                        
                });
                
               // == Highest rain reading in this zone (mm/h) ==================
                const sortRainSensor = zoneRainSensor.sort((a, b) =>
                    b.attributes.hora_1 - a.attributes.hora_1
                );
                ...
                const riverTendencia = worstRiverStation?.attributes.tendencia ?? null; // current status
                          
                const riverSituacao  = worstRiverStation?.attributes.situacao  ?? null; // rising / falling trend
                ...
                const supressed = await wasRecentlySent(zone.id, risk.severity);
                if (supressed) {
                    console.log(`[Zone] ${zone.name} → ⏸ Suppressed (cooldown)`);
                    continue
                };
                
                // == Build and broadcast the Telegram alert ==================
                const message =  buildMessage(zone.name, risk.severity, risk.reasons);
                ...
                for (const neighborhood of zone.neighborhoods) {
                    for (const user of neighborhood.users) {
                        if (user.isActive) {
                            // Bug fix: isActive was missing from schema — added it
                ...
                console.log(`[Zone] ${zone.name} → 🚨 ${risk.severity} sent to ${sent} user(s) | log #${alertLog.id}`);
